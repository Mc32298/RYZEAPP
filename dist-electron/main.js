...
function buildGoogleTokenExchangeDebugContext(args) {
  return {
...
function formatGoogleTokenExchangeError(status, errorText, debugContext) {
  ...
  if (normalized.includes("client_secret is missing") || parsedErrorDescription.toLowerCase().includes("client_secret is missing")) {
    return [
...
ipcMain.handle("gmail:move", async (_event, payload) => {
  ...
  const validDestinations = /* @__PURE__ */ new Set(["INBOX", "TRASH", "SPAM", "DRAFT", "ARCHIVE"]);
  if (!validDestinations.has(destination)) throw new Error("Invalid Gmail destination label");
  const accessToken = await getValidGoogleAccessToken(accountId);
  const { addLabelIds, removeLabelIds } = buildGmailMoveLabelMutation(destination);
  const response = await fetch(
    `https://gmail.googleapis.com/gmail/v1/users/me/messages/${encodeURIComponent(messageId)}/modify`,
    {
      method: "POST",
