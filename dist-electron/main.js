...
ipcMain.handle("microsoft-mail:mark-read", async (_event, payload) => {
  ...
  const tokens = getTokensByAccountId(accountId);
  if (tokens) {
    updateMessage(tokens.accessToken, messageId, { isRead }).catch(
      (e) => console.error("Failed background sync for mark-read:", e)
    );
  }
  return true;
});
...
function getTokensByAccountId(accountId, _authEnv) {
  const tokens = loadMicrosoftTokens();
  const token = tokens[accountId];
  if (!token || token.provider !== "microsoft" || token.accountId !== accountId) {
    return void 0;
  }
  return token;
}
async function updateMessage(accessToken, messageId, arg2) {
