...
function resolveMarkReadValue(value) {
  return typeof value === "boolean" ? value : true;
}
function buildGraphReplyPayload(comment) {
  return {
    comment: comment.trim() || " "
  };
}
function buildGoogleAuthorizationCodeParams(args) {
...
function buildGoogleRefreshTokenParams(args) {
  const params = new URLSearchParams({
    client_id: args.clientId,
...
function formatGoogleTokenExchangeError(status, errorText) {
  const normalized = errorText.toLowerCase();
  if (normalized.includes("client_secret is missing") || normalized.includes('"error":"invalid_request"')) {
    return [
...
const googleTokenFilePath = require$$1.join(
  app.getPath("userData"),
...
function getGraphRetryDelayMs(response, attempt) {
...
function saveFolderSyncState(accountId, folderId, deltaLink, syncType) {
...
function upsertLocalGraphMessage(accountId, folderId, message) {
  ...
  const fromAddress = message.from?.emailAddress?.address || message.sender?.emailAddress?.address || "";
  const isStarred = message.flag ? message.flag.flagStatus === "flagged" ? 1 : 0 : null;
  const isRead = message.isRead !== void 0 ? message.isRead ? 1 : 0 : null;
  const hasAttachments = message.hasAttachments !== void 0 ? message.hasAttachments ? 1 : 0 : null;
  const bodyContent = message.body?.content || message.bodyContent || "";
  const bodyContentType = message.body?.contentType || message.bodyContentType || "";
  db.prepare(
    `
    INSERT INTO emails (
      id,
...
function deleteLocalMessage(accountId, messageId) {
  db.transaction(() => {
    db.prepare(
      `
...
function validateFolderName(name) {
  const value = assertString(name, "folderName", 64).replace(/\s+/g, " ").trim();
  if (value.length < 2) {
...
const GOOGLE_CLIENT_ID = process.env.GOOGLE_OAUTH_CLIENT_ID?.trim() || "224714941754-rhbg9oqtj0vieilhj1p3fc4slai0ah09.apps.googleusercontent.com";
const GOOGLE_REDIRECT_URI = process.env.GOOGLE_OAUTH_REDIRECT_URI?.trim() || "http://127.0.0.1:42814/auth/google/callback";
...
function gmailParseHeader(headers, name) {
...
async function fetchGraphFolderMessagesDeltaPage(accessToken, url) {
  for (let attempt = 1; attempt <= maxGraphFetchAttempts; attempt += 1) {
    ...
    const errorText = await response.text();
    const shouldRetry = response.status === 429 || response.status === 500 || response.status === 502 || response.status === 503 || response.status === 504;
...
async function syncFolderInitialFullDelta(accessToken, accountId, folderId) {
  const query = new URLSearchParams({
    $select: "id,subject,bodyPreview,receivedDateTime,isRead,hasAttachments,from,toRecipients,ccRecipients,flag"
    // <--- Added ,flag
  });
  let nextUrl = `https://graph.microsoft.com/v1.0/me/mailFolders/${encodeURIComponent(
    folderId
...
async function syncFolderDelta(accessToken, accountId, folderId) {
  ...
  try {
    ...
    if (message.includes("410") || message.toLowerCase().includes("sync state") || message.toLowerCase().includes("deltalink")) {
...
async function fetchGraphMailFoldersPage(accessToken, url) {
  for (let attempt = 1; attempt <= maxGraphFetchAttempts; attempt += 1) {
    const response = await fetch(url, {
      headers: {
        Authorization: `Bearer ${accessToken}`
    ...
    const errorText = await response.text();
    if (response.status === 404 || response.status === 403 || errorText.includes("ErrorItemNotFound") || errorText.includes("ErrorAccessDenied")) {
      return { value: [] };
    }
...
async function createGraphMailFolder(accessToken, displayName) {
...
async function moveGraphMessageToFolder(accessToken, messageId, destinationFolderId) {
...
async function renameGraphMailFolder(accessToken, folderId, displayName) {
...
async function deleteGraphMailFolder(accessToken, folderId) {
  const encodedFolderId = encodeURIComponent(folderId);
  const response = await fetch(
    `https://graph.microsoft.com/v1.0/me/mailFolders/${encodedFolderId}`,
    {
...
async function deleteGraphMessage(accessToken, messageId) {
  const encodedMessageId = encodeURIComponent(messageId);
...
async function emptyGraphMailFolder(accessToken, folderId, isDeletedItemsFolder) {
...
async function fetchGraphMailFolders(accessToken) {
  const query = new URLSearchParams({
    $top: "100"
  });
  const rootUrl = `https://graph.microsoft.com/v1.0/me/mailFolders?${query.toString()}`;
  const allFolders = [];
  const fetchFolderTree = async (url, depth, parentPath) => {
    let nextUrl = url;
    while (nextUrl) {
      let page;
      try {
...
function getKnownFolderName(folder) {
  ...
  if (id === "inbox" || displayName === "inbox" || displayName === "indbakke" || displayName === "bandejadeentrada" || displayName === "boîtederéception")
    return "inbox";
  if (id === "sentitems" || displayName === "sentitems" || displayName === "sent" || displayName === "sendtpost" || displayName === "elementosenviados" || displayName === "élémentsenvoyés")
    return "sentitems";
  if (id === "drafts" || displayName === "drafts" || displayName === "kladder" || displayName === "borradores" || displayName === "brouillons")
    return "drafts";
  if (id === "archive" || displayName === "archive" || displayName === "arkiv" || displayName === "archivo")
    return "archive";
  if (id === "deleteditems" || displayName === "deleteditems" || displayName === "deleted" || displayName === "trash" || displayName === "papirkurv" || displayName === "elementoseliminados" || displayName === "élémentsupprimés") {
    return "deleteditems";
  }
  return "";
}
...
function loadWindowState() {
  try {
    if (fs$1.existsSync(stateFilePath)) {
      return JSON.parse(fs$1.readFileSync(stateFilePath, "utf8"));
    }
  } catch (error2) {
    console.error("Failed to load window state:", error2);
  }
...
function saveWindowState(win) {
  try {
    if (!win.isMaximized() && !win.isMinimized()) {
      ...
      fs$1.writeFileSync(stateFilePath, JSON.stringify(bounds), {
        encoding: "utf8",
        mode: 384
      });
    }
  } catch (error2) {
    console.error("Failed to save window state:", error2);
  }
}
...
ipcMain.handle("ai:summarize-email", async (_event, payload) => {
  ...
  if (getAiProvider() === "ollama") {
    const { baseUrl, model: model2 } = getOllamaConfig();
    const ollamaBody = limitAiInput(plainBody, 4e3);
    const ollamaPrompt = [
    ...
    const requestOllamaSummary = async (numPredict) => {
      const response2 = await fetch(`${baseUrl}/api/generate`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
      ...
      if (!response2.ok) {
        ...
        throw new Error(
          `Ollama summarize failed (${response2.status}): ${errorText}`
        );
      }
      return await response2.json();
    };
    let data2 = await requestOllamaSummary(500);
    let summary2 = (data2.response || data2.message?.content || data2.output || "").trim();
    if (!summary2 && data2.done_reason === "length") {
      data2 = await requestOllamaSummary(1e3);
      summary2 = (data2.response || data2.message?.content || data2.output || "").trim();
    }
    if (!summary2) {
      const reason = data2.error || data2.done_reason || "no response text";
      throw new Error(`Ollama returned an empty summary (${reason}).`);
    }
    return {
      ...normalizeAiSummaryResult(summary2)
    };
  }
...
ipcMain.handle("microsoft-mail:download-attachment", async (event, payload) => {
  ...
  if (!window2) throw new Error("No window found");
  ...
  const baseAttachmentUrl = `https://graph.microsoft.com/v1.0/me/messages/${encodedMsgId}/attachments/${encodedAttId}`;
  const valueUrl = `${baseAttachmentUrl}/$value`;
  const tryDownload = async (preferImmutableId) => {
  ...
  const secondAttempt = firstAttempt.ok || firstAttempt.status !== 404 ? null : await tryDownload(false);
  const successAttempt = firstAttempt.ok ? firstAttempt : secondAttempt?.ok ? secondAttempt : null;
  if (!successAttempt) {
    const failure = secondAttempt || firstAttempt;
    throw new Error(`Failed to download attachment: ${failure.errorText}`);
  }
  fs$1.writeFileSync(filePath, successAttempt.bytes);
...
ipcMain.handle("microsoft-mail:syncFolders", async (_event, payload) => {
  ...
  const folderIds = rawFolderIds.map(
...
ipcMain.handle("system:get-drafts", () => {
  try {
    ...
    console.error("Failed to load encrypted drafts:", error2);
    return [];
  }
});
...
ipcMain.handle("microsoft-folder:empty", async (_event, payload) => {
  ...
  const accessToken = await getValidMicrosoftAccessToken(accountId);
  const isDeletedItemsFolder = folder.wellKnownName === "deleteditems" || folder.displayName?.toLowerCase() === "deleted items" || folder.displayName?.toLowerCase() === "trash";
...
ipcMain.handle("microsoft-oauth:connect", async () => {
  ...
  const authCode = await new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (error2) {
        ...
        server.close(() => reject(new Error(errorDescription || error2)));
        return;
      }
    ...
    server.once("error", (error2) => reject(error2));
    ...
    server.listen(Number(redirect.port), redirect.hostname, () => {
      shell.openExternal(authUrl.toString()).catch((error2) => {
  ...
  tokens[accountId] = {
    ...
    expiresAt: Date.now() + tokenPayload.expires_in * 1e3,
...
ipcMain.handle("gmail:get-body", async (_event, payload) => {
  ...
  const folderId = (msg.labelIds ?? []).find(
    (l) => GMAIL_SYSTEM_FOLDERS.some((f) => f.id === l)
  ) ?? "INBOX";
...
ipcMain.handle("gmail:move", async (_event, payload) => {
  ...
  const validDestinations = /* @__PURE__ */ new Set(["INBOX", "TRASH", "SPAM", "DRAFT"]);
...
ipcMain.handle("microsoft-mail:reply", async (_event, payload) => {
  const accountId = validateAccountId(payload?.accountId);
  const messageId = validateMessageId(payload?.messageId);
  const comment = optionalString(payload?.comment, "comment", 5e5);
  const accessToken = await getValidMicrosoftAccessToken(accountId);
  const encodedMessageId = encodeURIComponent(messageId);
  const response = await fetch(
...
ipcMain.handle("microsoft-mail:getAllLocal", () => {
  ...
  for (const folder of folders) {
    messagesByFolder[folder.id] = rowsToMessages(
      emailRows.filter((r) => r.folder === folder.id)
    );
  }
  const labels = db.prepare(`SELECT * FROM labels ORDER BY accountId, name COLLATE NOCASE ASC`).all();
...
ipcMain.handle("microsoft-mail:get-body", async (_event, payload) => {
  ...
  const attachments = Array.isArray(message.attachments) ? message.attachments.filter(
    (attachment) => String(attachment?.["@odata.type"] || "").includes("fileAttachment")
  ).map((attachment) => ({
    id: attachment.id || "",
    ...
    contentId: attachment.contentId || void 0
...
ipcMain.handle("microsoft-mail:move-to-folder", async (_event, payload) => {
  ...
  const destinationFolder = db.prepare(
    `
      SELECT id
      FROM folders
      WHERE accountId = ? AND id = ?
...
ipcMain.handle("microsoft-mail:move", async (_event, payload) => {
  ...
  const accessToken = await getValidMicrosoftAccessToken(accountId);
  const encodedMessageId = encodeURIComponent(messageId);
  const targetFolderRow = db.prepare(
    `
    SELECT id FROM folders 
    WHERE accountId = ? AND wellKnownName = ? 
    LIMIT 1
  `
  ).get(accountId, destinationFolderKey);
...
app.whenReady().then(() => {
  ...
  if (app.isPackaged) {
    setTimeout(() => {
      mainExports$1.autoUpdater.checkForUpdates().catch((err) => {
